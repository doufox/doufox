...
class AttachmentController extends Admin {
    ...
    public function __construct() {
		parent::__construct();
		if (core::get_action_id() != 'ajaxswfupload' && !$this->session->get('user_id') && !$this->cookie->get('member_id')) $this->attMsg('无权限操作，请登录。');
		$this->dir = 'upload/';
	}
        ...
        if ($data) {
            foreach ($data as $t) {
                ...
                $list[] = array(
                    ...
                    'url'      => is_dir($this->dir . $path) ? url('admin/attachment/index', array('dir'=>$path, 'iframe'=>$iframe, 'admin'=>$admin)) : '',
                );
            }
        }
